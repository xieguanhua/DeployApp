...
export function useRouterPush(inSetup = true) {
  ...
  function getSafeRedirectUrl(redirectUrl: string) {
    if (!redirectUrl || isLoginPath(redirectUrl)) {
      return '/';
    }
    return redirectUrl;
  }
  ...
  async function toLogin(loginModule?: UnionKey.LoginModule, redirectUrl?: string) {
    ...
    const currentRedirect = route.value.query?.redirect as string | undefined;
    const rawRedirect =
      redirectUrl ||
      (String(route.value.name || '') === 'login' ? currentRedirect || '' : route.value.fullPath);
