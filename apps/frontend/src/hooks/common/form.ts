...
export function useFormRules() {
  const patternRules = {
    ...
    phoneOrEmail: {
      validator: (_rule: App.Global.FormRule, value: string) => {
        ...
        if (REG_PHONE.test(text) || REG_EMAIL.test(text)) return Promise.resolve();
        return Promise.reject(new Error($t('form.phoneOrEmail.invalid')));
      },
      trigger: 'change'
    }
  ...
  const formRules = {
    ...
    phoneOrEmail: [createRequiredRule($t('form.phoneOrEmail.required')), patternRules.phoneOrEmail]
  } satisfies Record<string, App.Global.FormRule[]>;
