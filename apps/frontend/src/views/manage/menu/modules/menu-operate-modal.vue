...
import { fetchUpsertMenu } from '@/service/api';
import {
  getLayoutAndPage,
  ...
  routeName: [
    defaultRequiredRule,
    {
      validator: (_rule: any, value: string) => {
        if (!value) {
          return Promise.reject(new Error($t('page.manage.menu.form.routeName')));
        }

        if (!props.allPages.includes(value)) {
          return Promise.reject(new Error($t('page.manage.menu.routeNameInvalid')));
        }
